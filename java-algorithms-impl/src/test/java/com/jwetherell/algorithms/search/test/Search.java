...
public class Search {
    ...
    public void testBruteForceSearch() {
        ...
        AllTests.assertTrue("Brute force error. expected=" + valueIndex + " got=" + index, (index == valueIndex));
        index = LinearSearch.find(valueNotInArray, sorted);
        AllTests.assertTrue("Brute force error. expected=" + Integer.MAX_VALUE + " got=" + index, (index == Integer.MAX_VALUE));
    }
    ...
    public void testLowerBound() {
        ...
        AllTests.assertTrue("Lower bound error. Sequences size=" + sequences.size() + " sough elements size=" + soughtElements.size() + " results size:" + resultIndexes.size(), sequences.size() == resultIndexes.size() && sequences.size() == soughtElements.size());

        for (int i = 0; i < sequences.size(); ++i) {
            int resultIndex = LowerBound.lowerBound(sequences.get(i), sequences.get(i).length, soughtElements.get(i));
            AllTests.assertTrue("Lower bound error. Sequence=" + Arrays.toString(sequences.get(i)) + " sought element=" + soughtElements.get(i) + " expected result=" + resultIndexes.get(i) + " got result=" + resultIndex, resultIndex == resultIndexes.get(i));
        }
    ...
    public void testUpperBound() {
        ...
        AllTests.assertTrue("Upper bound error. Sequences size=" + sequences.size() + " sough elements size=" + soughtElements.size() + " results size:" + resultIndexes.size(), sequences.size() == resultIndexes.size() && sequences.size() == soughtElements.size());

        for (int i = 0; i < sequences.size(); ++i) {
            int resultIndex = UpperBound.upperBound(sequences.get(i), sequences.get(i).length, soughtElements.get(i));
            AllTests.assertTrue("Upper bound error. Sequence=" + Arrays.toString(sequences.get(i)) + " sought element=" + soughtElements.get(i) + " expected result=" + resultIndexes.get(i) + " got result=" + resultIndex, resultIndex == resultIndexes.get(i));
        }
