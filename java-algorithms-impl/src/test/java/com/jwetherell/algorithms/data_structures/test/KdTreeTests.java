...
public class KdTreeTests {
    ...
    public void testKdTree() {
        ...
        AllTests.assertTrue("K-D Tree query error. query=(k=1, p=(9, 6)) returned="+result, result.contains(p3));

        XYZPoint search = new XYZPoint(1, 4);
        result = kdTree.nearestNeighbourSearch(4, search);
        AllTests.assertTrue("K-D Tree query error. query=(k=4, p=(1, 4)) returned="+result, (result.contains(p1) &&
                                                                                    result.contains(p2) &&
                                                                                    result.contains(p4) &&
                                                                                    result.contains(p6))
        );
